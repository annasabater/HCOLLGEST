 ...
 *   ⛔ FALTA: el ORDEN y la ESTRUCTURA exactos de los campos -> ver FIELD_LAYOUT.
 *      Eso solo está en el "Manual d'instruccions de l'usuari" del portal
 *      (apartado "Documentació i enllaços", sección de fitxers massius).
 *      En cuanto se rellene FIELD_LAYOUT con el orden del manual, queda 100% listo.
 */
...
export type TipusPagament =
  | 'DESTINACIO' | 'EFECTIU' | 'MOBIL' | 'PLATAFORMA'
  | 'TARGETA_CREDIT' | 'TRANSFERENCIA' | 'TARGETA_REGAL';
...
export const CONFIG = {
  separador: '|',
  saltLinia: '\r\n',
  // ⚠ CONFIRMAR EN EL MANUAL: la app es Java legacy -> probablemente ISO-8859-1.
  encoding: 'latin1' as 'latin1' | 'utf-8',
  formatData: (d: Date) =>
    `${String(d.getDate()).padStart(2, '0')}/` +
    `${String(d.getMonth() + 1).padStart(2, '0')}/` +
    `${d.getFullYear()}`, // ⚠ CONFIRMAR formato exacto (dd/mm/aaaa vs aaaammdd)
};

/** Códigos LITERALES que espera el fichero por cada enum.
 *  ⚠ Los valores de la derecha son provisionales: el manual indica el código
 *  exacto (p.ej. puede ser "D"/"N"/"P", "1"/"2", etc.). Ajustar al confirmarlos. */
export const CODES = {
  tipusRegistre: { CONTRACTE_EN_CURS: 'C', RESERVA: 'R' },              // TODO
  tipusDocument: { DNI_NIF: 'D', NIE: 'N', PASSAPORT: 'P', ALTRES: 'A' },// TODO
  sexe: { HOME: 'H', DONA: 'D' },                                       // TODO
  tipusPagament: {                                                      // TODO
    DESTINACIO: '', EFECTIU: '', MOBIL: '', PLATAFORMA: '',
    TARGETA_CREDIT: '', TRANSFERENCIA: '', TARGETA_REGAL: '',
  },
...
// ----------------------------------------------------------------------------
// 3. LAYOUT DEL FICHERO  ⛔ RELLENAR CON EL ORDEN EXACTO DEL MANUAL
// ----------------------------------------------------------------------------
/**
 ...
 */
type FieldDef = { name: string; value: (p: ParteViatgers, v: Viatger) => string };
...
function buildLines(parte: ParteViatgers): string[] {
  if (FIELD_LAYOUT.length === 0) {
    throw new Error('FIELD_LAYOUT vacío: falta el orden de campos del manual.');
  }
  return parte.viatgers.map((v) =>
    FIELD_LAYOUT
      .map((f) => formatField(f.value(parte, v)))
      .join(CONFIG.separador), // los campos vacíos conservan su "|" automáticamente
  );
}
...
/** Devuelve el contenido del fichero como string. Codificar al escribir a disco
 *  con CONFIG.encoding: fs.writeFileSync(name, content, { encoding: CONFIG.encoding }) */
...
// ----------------------------------------------------------------------------
// 5. VALIDACIÓN DE OBLIGATORIEDAD CONDICIONAL (reglas oficiales)
// ----------------------------------------------------------------------------
...
export function validaParte(parte: ParteViatgers): void {
  ...
  parte.viatgers.forEach((v, i) => {
    ...
    if (v.esMenor && !v.parentesc?.trim())
      errs.push(`${p}: falta parentesc (menor)`);
