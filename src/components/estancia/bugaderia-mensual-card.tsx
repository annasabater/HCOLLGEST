    ...
    <Card>
      ...
      <CardBody>
        ...
        <p className="mb-3 text-xs text-slate-400">
          Estimat segons els articles marcats a les estades que fan <strong>sortida</strong> aquest mes. Serveix per comprovar la factura de la bugaderia.
        </p>
        {loading ? (
          <p className="text-sm text-slate-400">Carregant…</p>
        ) : detall.length === 0 ? (
          <p className="text-sm text-slate-400">Cap sortida amb bugaderia marcada aquest mes.</p>
        ) : (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-slate-200 text-xs uppercase text-slate-400">
                  ...
                  <th className="py-1.5 text-right font-medium">Manteniment</th>
              ...
              <tbody>
                {detall.map((r) => (
                  <tr key={r.id} className="border-b border-slate-50">
                    ...
                    <td className="py-1.5 text-right text-slate-500"><Eur value={r.manteniment} /></td>
                    <td className="py-1.5 text-right text-slate-500"><Eur value={r.sortida} /></td>
