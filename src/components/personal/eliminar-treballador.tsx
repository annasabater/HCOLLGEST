...
export function EliminarTreballador({ id, nom }: { id: string; nom: string }) {
  ...
  return (
    <>
      <Button type="button" variant="ghost" size="sm" onClick={() => setObert(true)} disabled={busy} title="Eliminar">
        <Trash2 className="h-4 w-4 text-red-600" />
      </Button>
      <ConfirmDialog
        open={obert}
        title={`Eliminar el treballador «${nom}»?`}
        message="Es conservaran les jornades i nòmines històriques, però deixarà d'aparèixer a la llista."
        onConfirm={confirmar}
