import { NextResponse } from 'next/server';
import { prisma } from '@/lib/db';
import { authorize, clientIp } from '@/lib/auth/guard';
import { audit } from '@/lib/audit';
import { handleApiError, notFound } from '@/lib/http';
import { buildReglamentPdf } from '@/lib/pdf/reglament';

const ESTABLIMENT_ID = 'hostal-coll';
type Ctx = { params: Promise<{ id: string }> };

// GET /api/estancies/:id/reglament-pdf — Reglament intern d'hospedatge + LOPD (PDF amb firma)
export async function GET(req: Request, ctx: Ctx) {
  try {
    const auth = await authorize();
    if (auth instanceof Response) return auth;
    const { id } = await ctx.params;

    const estancia = await prisma.estancia.findFirst({ where: { id, deletedAt: null } });
    if (!estancia) return notFound();

    const [establiment, tots] = await Promise.all([
      prisma.establiment.findUniqueOrThrow({ where: { id: ESTABLIMENT_ID } }),
      prisma.estanciaViatger.findMany({
        where: { estanciaId: id },
        include: { huesped: true, signatura: true, habitacioSeparada: { select: { nom: true } } },
        orderBy: { esTitular: 'desc' },
      }),
    ]);

    // Filtre opcional ?hab=principal | ?hab=<nom>: només els viatgers d'aquell "contracte".
    const habFiltre = new URL(req.url).searchParams.get('hab');
    const viatgers = habFiltre
      ? tots.filter((v) =>
          habFiltre === 'principal' ? !v.habitacioSeparada : v.habitacioSeparada?.nom === habFiltre,
        )
      : tots;

    const pdf = await buildReglamentPdf(establiment, estancia, viatgers);

    const filename = `Reglament intern - ${estancia.numContracte}-${estancia.anyContracte}.pdf`;

    await audit({
      usuariId: auth.id,
      accio: 'IMPRESSIO',
      entitat: 'estancia',
      entitatId: id,
      detall: { document: 'reglament-pdf' },
      ip: clientIp(req),
    });

    return new NextResponse(new Uint8Array(pdf), {
      headers: {
        'Content-Type': 'application/pdf',
        'Content-Disposition': `inline; filename="${filename}"`,
        'Cache-Control': 'private, no-store',
      },
    });
  } catch (err) {
    return handleApiError(err);
  }
}

export const dynamic = 'force-dynamic';
